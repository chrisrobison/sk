...
show_usage() {
    ...
    echo ""
    echo "Convert FLAC files to WAV format"
    echo ""
    echo "Options:"
...
convert_flac_file() {
    ...
    # Generate output filename by replacing .flac with .wav
    local wav_file="${flac_file%.flac}.wav"
    
    echo "Converting '$flac_file' to '$wav_file'..."
    
    # Convert using ffmpeg
    if ffmpeg -i "$flac_file" -c:a pcm_s16le "$wav_file" -y 2>/dev/null; then
